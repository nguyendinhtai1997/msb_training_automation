...
[TC_01] - Tìm kiếm MSB trên google    # Tổng trừ    -1.1
    ...
    Sleep    2s    # đã có wait ở keyword dưới    - 0.1
    # class động KHÔNG NÊN DÙNG    -0.2    //h3[1]
    Click kết quả tìm kiếm đầu tiên
    ...    //div[@class="tF2Cxc"]//h3[@class="LC20lb MBeuO DKV0Md"]
    Sleep    4s    # đã có wait ở keyword dưới    - 0.1
    # //span[@class = "pop-up-porlet-close-btn"]    -0.1
    Đóng popup
    ...    //*[@id="fragment-34202-joci"]/div[1]/div[1]/div[1]/span[1]
    Sleep    2s    # đã có wait ở keyword dưới    - 0.1
